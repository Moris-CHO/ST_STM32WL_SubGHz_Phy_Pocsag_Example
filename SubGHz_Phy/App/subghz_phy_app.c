...
/**
  * @brief App_Process implementation
  */
...
static void OnTxDone(void)
{
  ...
  APP_LOG(TS_OFF, VLEVEL_L, "OnTxDone\n\r");
  /* Update the State of the FSM*/
  State = TX;
  /* Run App_Process in background*/
...
static void OnTxTimeout(void)
{
  ...
  APP_LOG(TS_OFF, VLEVEL_L, "OnTxTimeout\n\r");
  /* Update the State of the FSM*/
  State = TX_TIMEOUT;
  /* Run App_Process in background*/
  UTIL_SEQ_SetTask((1 << CFG_SEQ_Task_SubGHz_Phy_App_Process), CFG_SEQ_Prio_0);
...
static void OnRxDone(uint8_t *payload, uint16_t size, int16_t rssi, int8_t LoraSnr_FskCfo)
{
  ...
  APP_LOG(TS_OFF, VLEVEL_L, "OnRxDone\n\r");
#if ((USE_MODEM_LORA == 1) && (USE_MODEM_FSK == 0))
  ...
  APP_LOG(TS_OFF, VLEVEL_L, "RssiValue=%d dBm, Cfo=%dkHz\n\r", rssi, LoraSnr_FskCfo);
  ...
  {
    APP_LOG(TS_OFF, VLEVEL_L, "0x%02X ", BufferRx[i]);
    if (i % 10 == 9)
    {
...
static void OnRxTimeout(void)
{
  ...
  APP_LOG(TS_OFF, VLEVEL_L, "OnRxTimeout\n\r");
  /* Update the State of the FSM*/
  State = RX_TIMEOUT;
  /* Run App_Process in background*/
  UTIL_SEQ_SetTask((1 << CFG_SEQ_Task_SubGHz_Phy_App_Process), CFG_SEQ_Prio_0);
...
static void OnRxError(void)
{
  ...
  APP_LOG(TS_OFF, VLEVEL_L, "OnRxError\n\r");
  /* Update the State of the FSM*/
  State = RX_ERROR;
  /* Run App_Process in background*/
  UTIL_SEQ_SetTask((1 << CFG_SEQ_Task_SubGHz_Phy_App_Process), CFG_SEQ_Prio_0);
